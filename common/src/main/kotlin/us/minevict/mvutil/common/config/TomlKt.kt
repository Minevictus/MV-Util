...
data class TomlKt(internal val toml: Toml) {
    ...
    fun getTables(key: String): List<TomlKt>? = toml.getTables(key)?.map(::TomlKt)?.toList()
    fun getTables(key: String, default: List<TomlKt>): List<TomlKt> = getTables(key) ?: default
